...
public:
  ...
  explicit DataAggregator(std::vector<StringRef> Filenames)
      ...
      for (unsigned int i=0; i<Filenames.size(); i++){
        PerfProcessInfo tmp1;
        PerfProcessInfo tmp2;
        PerfProcessInfo tmp3;
        PerfProcessInfo tmp4;
        MMapEventsPPIs.push_back(tmp1);
        MemEventsPPIs.push_back(tmp2);
        TaskEventsPPIs.push_back(tmp3);
        MainEventsPPIs.push_back(tmp4);
      }
      start();
  }
...
private:
  ...
  std::vector<PerfProcessInfo> MainEventsPPIs;
  std::vector<PerfProcessInfo> MemEventsPPIs;
  std::vector<PerfProcessInfo> MMapEventsPPIs;
  std::vector<PerfProcessInfo> TaskEventsPPIs;
  ...
  /// parsing.
  void launchPerfProcess(StringRef Name, std::vector<PerfProcessInfo> &PPI,
